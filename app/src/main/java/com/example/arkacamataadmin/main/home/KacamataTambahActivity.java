...
public class KacamataTambahActivity extends AppCompatActivity implements View.OnClickListener {
    String nama_kacamata, harga_kacamata, deskripsi_kacamata, foto_kacamata, file_3d,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_kacamata_tambah);
        ...
        post.enqueue(new Callback<ResponseBody>(){
            ...
            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                ...
                try {
                    ...
                    sp_kategori.setAdapter(new ArrayAdapter<String>(KacamataTambahActivity.this, android.R.layout.simple_spinner_dropdown_item, list_kategori_nama));
                } catch (IOException e) {
                    e.printStackTrace();
                    Toast.makeText(KacamataTambahActivity.this, "Tidak bisa mengirim data!", Toast.LENGTH_LONG).show();
            ...
            public void onImagesPicked(@NonNull List<File> list, EasyImage.ImageSource imageSource, int type) {
                switch (type){
                    case REQUEST_CODE_GALLERY:
                        Glide.with(KacamataTambahActivity.this)
    ...
    private void simpan(){
        ...
        if (TextUtils.isEmpty(urlPath_3d)){
            et_file_3d.setError("Silahkann diisi..");
            focusView = et_file_3d;
            cancel = true;
        }
        ...
        if (cancel) {
            ...
            if (TextUtils.isEmpty(urlPath_foto)){
                ...
                UserAPIServices api = Config.getRetrofit(Config.URL).create(UserAPIServices.class);
                Call<ResponseBody> post = api.kacamata_tambah(requestBody);
                post.enqueue(new Callback<ResponseBody>(){
                    @Override
                    public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                        pDialog.dismiss();
                        try {
                            ...
                            for(int i=0;i<Config.jsonArray.length();i++) {
                                ...
                                if (status.equals("1")) {
                                    Toast.makeText(getApplicationContext(), "Berhasil Menambahkan Kacamata.", Toast.LENGTH_LONG).show();
                                    onBackPressed();
                                } else {
                                    Toast.makeText(getApplicationContext(), "Gagal Menambahkan Kacamata.", Toast.LENGTH_LONG).show();
                                }
                            }
                        } catch (IOException e) {
                            e.printStackTrace();
